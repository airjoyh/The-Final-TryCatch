							...
							<table cellspacing='0'>
								...
								<c:forEach items="${list }" var="note" varStatus="stat">
									<tr>
										...
										<td style="width: 100%; display: table-cell;"><a
											href="${initParam.rootPath }/company/note/read${pageMaker.makeSearch(pageMaker.cri.page)}&note_receiver=${note.note_receiver }&note_id=${note.note_id }&note_status=${note.note_status}">${note.note_contents}</a></td>
										<td>${note.note_sender}</td>
										<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm"
												value="${note.note_senddate}" /></td>
									</tr>
								</c:forEach>
							</table>
						</div>
